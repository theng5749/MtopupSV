...
Partial Class discount
    ...
    Sub _gridColumnName()
        If gridDiscount.GridControlElement.Rows.Count <> 0 Then
            ...
            gridDiscount.GridControlElement.HeaderRow.Cells(6).Text = "ເປີເຊັນສ່ວນຫຼຸດ (%)"
            gridDiscount.GridControlElement.HeaderRow.Cells(8).Text = "ບັນທຶກຂໍ້ມູນໂດຍ"
        End If
    ...
    Private Sub FillData()
        Dim sql As String = ""
        sql = "select  * " _
            & " from mtopup.tbl_discount order by credit_start asc"
    ...
    Protected Sub btConfirmSave2_ServerClick(sender As Object, e As EventArgs) Handles btConfirmSave2.ServerClick
        ...
        If txtCreditPercent.Value.Trim = "" Then
            _Alert()
            lbAlertInfo.Text = "ກະລຸນາປ້ອນເປີເຊັນສ່ວນຫຼຸດ"
            alertInfo.Visible = True
        ...
        Catch ex As Exception
            _Alert()
            lbAlertInfo.Text = "ເປີເຊັນສ່ວນຫຼຸດບໍ່ຖືກຕ້ອງ, ກວດສອບຄືນໃໝ່"
            alertInfo.Visible = True
            txtCreditPercent.Focus()
            Exit Sub
        End Try
    ...
    Public Function _addDiscount() As Boolean
        ...
        Try
            Dim cm As OracleCommand = New OracleCommand(myDataType.sqlInsertDiscount, cn) With {.CommandType = CommandType.StoredProcedure}
            cm.Parameters.Add("p_credit_start", OracleDbType.Int32).Value = CDbl(txtCreditStart.Value.Trim.Replace(",", ""))
            cm.Parameters.Add("p_credit_stop", OracleDbType.Int32).Value = CDbl(txtCreditStop.Value.Trim.Replace(",", ""))
            cm.Parameters.Add("p_discount_amt", OracleDbType.Int32).Value = CDbl(txtCreditPercent.Value.Trim.Replace(",", ""))
            ...
            fc.saveLog(Session("userid").ToString, "Add new discount - [start:" & txtCreditStart.Value.Trim & "] [stop:" & txtCreditStop.Value.Trim & "] [percent:" & txtCreditPercent.Value.Trim & "]")
            _return = True
        Catch ex As Exception
            _Alert()
    ...
    Public Function _deleteDiscount() As Boolean
        ...
        End Try
        Try
            Dim cm As OracleCommand = New OracleCommand(myDataType.sqlDeleteDiscount, cn) With {.CommandType = CommandType.StoredProcedure}
            cm.Parameters.Add("p_credit_start", OracleDbType.Int32).Value = CDbl(gridDiscount.GridControlElement.SelectedRow.Cells(4).Text.Trim.Replace(",", ""))
            cm.Parameters.Add("p_credit_stop", OracleDbType.Int32).Value = CDbl(gridDiscount.GridControlElement.SelectedRow.Cells(5).Text.Trim.Replace(",", ""))
            cm.Parameters.Add("p_discount_amt", OracleDbType.Int32).Value = gridDiscount.GridControlElement.SelectedRow.Cells(6).Text.Trim.Replace(",", "")
            cm.ExecuteNonQuery()
            fc.saveLog(Session("userid").ToString, "Delete discount - [start:" & gridDiscount.GridControlElement.SelectedRow.Cells(4).Text.Trim & "] [stop:" & gridDiscount.GridControlElement.SelectedRow.Cells(5).Text.Trim & "] [percent:" & gridDiscount.GridControlElement.SelectedRow.Cells(6).Text.Trim & "]")
